<script>
  ...
  let selected = false
  $: remainders = Array.from({length: items.length}, (_, i) => i - currentStep)
  let carrouselItems = []
...
<div class={`${classes.carrouselWrapper} ${className}`}>
  <List {items} let:prop={item} className={classes.carrousel} on:scroll={getCurrentStep}>
    <div bind:this={carrouselItems[items.indexOf(item)]} class={`${classes.carrouselItem} ${items.indexOf(item)%2===0? '-': ''}mt-32`}>
      <CarrouselItem
        {...item}
        on:preview={() => {
